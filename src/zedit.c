...
void zedit_setup(struct descriptor_data *d, int room_num)
{
  ...
  zone->development = zone_table[OLC_ZNUM(d)].development;
...
void zedit_new_zone(struct char_data *ch, int vzone_num)
{
  ...
  if (vzone_num < 0) {
    ...
    return;
  } else if (vzone_num > 654) {
    send_to_char("654 is the highest zone allowed.\r\n", ch);
    return;
  }
...
void zedit_save_internally(struct descriptor_data *d)
{
  ...
  if (OLC_ZONE(d)->number) {
    ...
    zone_table[OLC_ZNUM(d)].development = OLC_ZONE(d)->development;
...
void zedit_save_to_disk(int zone_num)
{
  ...
  fprintf(zfile, "#%d\n" "%s~\n" "%d %d %d %d %d\n",
	  ...
	  zone_table[zone_num].reset_mode,
          zone_table[zone_num].development,
  ...
  sprintf(buf,
	...
	grn, nrm, yel, OLC_ZONE(d)->name ? OLC_ZONE(d)->name : "<NONE!>",
	  ...
	  ((OLC_ZONE(d)->reset_mode == 1) ?
	   ...
	   "Normal reset.") :
	  "Never reset", nrm,
	  grn, nrm, yel, YESNO(OLC_ZONE(d)->development), nrm
	  );
...
void zedit_disp_arg3(struct descriptor_data *d)
{
  ...
  case 'D':
    ...
    break;
  case 'G':
    send_to_char("Give the percentage chance that this event should happen: ",
                 d->character);
    break;
  case 'V':
...
*/
    ...
    case 'p':
    case 'P':
      send_to_char("Do you wish for this zone to be 'Portal Proof'?: ", d->character);
      OLC_MODE(d) = ZEDIT_DEVELOPMENT;
      break;
    default:
      zedit_disp_menu(d);
      break;
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ZONE_TOP:
    ...
    zedit_disp_menu(d);
    break;
  case ZEDIT_DEVELOPMENT:
    /* Set development mode */
    switch (*arg) {
      case 'y':
      case 'Y':
        OLC_ZONE(d)->development=TRUE;
        break;
      default:
        OLC_ZONE(d)->development=FALSE;
        break;
    }
    OLC_ZONE(d)->number = 1;
    zedit_disp_menu(d);
