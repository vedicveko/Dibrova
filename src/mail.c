...
void postmaster_send_mail(struct char_data * ch, struct char_data *mailman,
			  int cmd, char *arg);
void clan_postmaster_send_mail(struct char_data * ch, struct char_data *mailman,
                               int cmd, char *arg);
...
/* THESE TWO LINES ADDED FOR MAIL_ALL */
...
SPECIAL(clan_mail)
{
  ...
  if (CMD_IS("order")) {
    ...
    if(GET_CLAN_RANK(ch) < 6) {
      send_to_char("Only clan leaders can use this function.\r\n", ch);
       return 0;
    }
    if (no_mail) {
      send_to_char("Sorry, the clan order system is currently down.\r\n", ch);
      return 0;
    }
 
    argument = "Blackadder";

    send_to_char("\r\n  /cw        ***PLEASE PLACE YOUR ORDER BELOW***/c0\r\n", ch);
    send_to_char("\r\n    Please be certain to specify all facets of your \r\n", ch);
    send_to_char("    order including name, description, stats (if obj)\r\n", ch);
    send_to_char("    and what you believe the total price to be according\r\n", ch);
    send_to_char("    to 'help clanprices'.\r\n\r\n\r\n", ch);

    clan_postmaster_send_mail(ch, me, cmd, argument);
  
    return 1;
  }
  if (CMD_IS("petition")) {
    if (no_mail) {
      send_to_char("Sorry, the petition system is currently down.\r\n", ch);
      return 0;
    }

    argument = "Kaan";

    send_to_char("\r\n/cW***PLEASE SUBMIT YOUR PETITION BELOW***/c0\r\n",
ch);
    send_to_char("\r\n/cwPlease be certain to specify your name, the\r\n", ch);
    send_to_char("name of the leader you wish removed, and the  \r\n", ch);
    send_to_char("reason why you wish them removed./c0\r\n", ch);
