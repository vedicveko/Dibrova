...
int isname(char *str, char *namelist)
{
	...
	if (arg2[i] == ' ' || arg2[i] == '\0') {
		...
		return 1;	/* Stop looking. and return true */
	    wp = 0;
	} else {
	    wrd[wp++] = arg2[i];
	}
...
/* Decide if arg1 is an abbreviation of arg2 */
/* Utility func for new ver of isname(...) */
int isabbrev(char *arg1, char *arg2)
{
    char *p1 = arg1;
    char *p2 = arg2;
...
void affect_total(struct char_data *ch)
{
  ...
  if (IS_NPC(ch))
    i = 25;
  else if (GET_LEVEL(ch) < 50 && GET_CLASS(ch) < CLASS_VAMPIRE)
    i = 18;
  else if (GET_LEVEL(ch) < 75 && GET_CLASS(ch) < CLASS_VAMPIRE)
    i = 21;
  else if (GET_LEVEL(ch) < 100 && GET_CLASS(ch) < CLASS_VAMPIRE)
    i = 23;
  else if (GET_LEVEL(ch) < 125 && GET_CLASS(ch) < CLASS_VAMPIRE)
    i = 24;
  else			/* 125+ */
    i = 25;
  ...
  /* Race-based perma-affects */
  if ((!IS_NPC(ch))
	&& (GET_RACE(ch) == RACE_VALKYRIE || GET_RACE(ch) == RACE_AVIADES
         || GET_RACE(ch) == RACE_SPRITE || GET_RACE(ch) == RACE_HARPY
         || GET_RACE(ch) == RACE_PIXIE))
    SET_BIT_AR(AFF_FLAGS(ch), AFF_FLY);

  if (!IS_NPC(ch) && GET_RACE(ch) == RACE_TRITON)
	SET_BIT_AR(AFF_FLAGS(ch), AFF_BREATHE);

  if (!IS_NPC(ch)
	&& (GET_RACE(ch) == RACE_ELF || GET_RACE(ch) == RACE_WOLFEN
	    || GET_RACE(ch) == RACE_DRIDER || GET_RACE(ch) == RACE_GNOME
	    || GET_CLASS(ch) == CLASS_VAMPIRE))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_INFRAVISION);

  if (!IS_NPC(ch) && GET_RACE(ch) == RACE_TROLL)
	SET_BIT_AR(AFF_FLAGS(ch), AFF_REGENERATE);

  if (!IS_NPC(ch) && GET_RACE(ch) == RACE_KENDER)
	SET_BIT_AR(AFF_FLAGS(ch), AFF_SEE_HIDDEN);

  if (!IS_NPC(ch) && GET_RACE(ch) == RACE_GOBLIN)
	SET_BIT_AR(AFF_FLAGS(ch), AFF_SNEAK);

  if (!IS_NPC(ch) && GET_RACE(ch) == RACE_NYMPH)
	SET_BIT_AR(AFF_FLAGS(ch), AFF_WATERWALK);

  if (!IS_NPC(ch) && GET_RACE(ch) == RACE_NAGA)
	SET_BIT_AR(AFF_FLAGS(ch), AFF_DETECT_ALIGN);

  if (!IS_NPC(ch) && GET_CLASS(ch) == CLASS_ASSASSIN)
        SET_BIT_AR(AFF_FLAGS(ch), AFF_SNEAK);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_VAMPIRE))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_DETECT_INVIS);

  if (!IS_NPC(ch)
	&& (GET_RACE(ch) == RACE_GIANT
	    || GET_CLASS(ch) == CLASS_VAMPIRE)) SET_BIT_AR(AFF_FLAGS(ch),
							   AFF_SENSE_LIFE);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_VAMPIRE))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_PROTFROMGOOD);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_SAINT))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_PROTECT_EVIL);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_VAMPIRE))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_HASTE);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_VAMPIRE))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_VAMP_AURA);

  if (!IS_NPC(ch) && (GET_CLASS(ch) >= CLASS_VAMPIRE))
	REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_SANCTUARY);

  if (!IS_NPC(ch) && (GET_CLASS(ch) >= CLASS_VAMPIRE))
    REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_DARKWARD);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_TITAN))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_BALANCE);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_TITAN))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_AWARE);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_TITAN))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_REGENERATE);

  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_TITAN))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_EARTH_SHROUD);
  ...
  if (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_DEMON))
	SET_BIT_AR(AFF_FLAGS(ch), AFF_NETHERBLAZE);
...
void affect_from_char(struct char_data *ch, int num, byte type)
{
    struct affected_type *hjp, *next;

    for (hjp = ch->affected; hjp; hjp = next) {
	next = hjp->next;
	if ((hjp->src == type) && (hjp->type == num))
 ...
 * not affected
 */
bool affected_by_spell(struct char_data *ch, int spell, byte type)
{
    struct affected_type *hjp;

    for (hjp = ch->affected; hjp; hjp = hjp->next)
	if ((hjp->src == type) && (hjp->type == spell))
	    return TRUE;
...
void obj_from_char(struct obj_data *object)
{
    ...
    if(GET_OBJ_WEIGHT(object) < 0) {
/*      sprintf(buf, "WEIGHT: NAME: %s OBJ:%s WEIGHT: %d", GET_NAME(object->carried_by),
              object->short_description,
              GET_OBJ_WEIGHT(object));
      log(buf); */
      if(GET_OBJ_TYPE(object) == ITEM_CONTAINER) {
         GET_OBJ_WEIGHT(object) = number(5, 15);
      }
    }
    if(IS_CARRYING_W(object->carried_by) < 0) {
/*        sprintf(buf, "WEIGHT: PC: %s OBJ: %s",
                GET_NAME(object->carried_by), object->short_description);
        log(buf); */
        IS_CARRYING_W(object->carried_by) = number(5, 15);
    }
    else {
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
    ...
    if (GET_EQ(ch, pos)) {
	sprintf(buf, "SYSERR: Char is already equipped: %s, %s",
    ...
    if(!obj || obj == NULL) {
      sprintf(buf, "NULL obj passed to equip_char by %s", GET_NAME(ch));
      log(buf);
    }
    ...
    if (!IS_NPC(ch)) {
	if (obj->obj_flags.minlevel > GET_LEVEL(ch)
	    && GET_CLASS(ch) < CLASS_VAMPIRE) {
	    act("You are zapped by $p! Your level is too low.", FALSE, ch,
		obj, 0, TO_CHAR);
	    act("$n is zapped by $p and instantly lets go of it.", FALSE,
		ch, obj, 0, TO_ROOM);
	    obj_to_char(obj, ch);	/* changed to drop in inventory instead of
					 * ground */
	    return;
	...
	if (obj->obj_flags.clan != -1) {
	    act("You are zapped by $p, you aren't of that clan!", FALSE,
		ch, obj, 0, TO_CHAR);
	    act("$n is zapped by $p and instantly lets go of it.", FALSE,
		ch, obj, 0, TO_ROOM);
	    obj_to_char(obj, ch);
...
/* search a room for a obj, and return a pointer if found..  */
struct obj_data *get_obj_room(char *name, obj_rnum room)
{
    struct obj_data *i;
    int j = 0, number;
    char tmpname[MAX_INPUT_LENGTH];
    ...
    for (i = world[room].contents; i && (j <= number); i = i->next_content)
	if (isname(tmp, i->short_description))
	    if (++j == number)
		return i;
...
void obj_from_room(struct obj_data *object)
{
	...
	log
	    ("SYSERR: NULL object or obj not in a room passed to obj_from_room");
	return;
    }

    REMOVE_FROM_LIST(object, world[object->in_room].contents,
		     next_content);
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to)
{
	...
	log
	    ("SYSERR: NULL object or same source and target obj passed to obj_to_obj");
	return;
    }
    ...
    if (tmp_obj->carried_by) {
        if(IS_CARRYING_W(tmp_obj->carried_by) < 0) {
/*            sprintf(buf, "WEIGHT: PC: %s OBJ: %s OBJFROM: %s",
                    GET_NAME(tmp_obj->carried_by), obj->short_description,
                    tmp_obj->short_description);
            log(buf); */
            IS_CARRYING_W(tmp_obj->carried_by) = number(5, 15);
        }
        else {
...
void obj_from_obj(struct obj_data *obj)
{
    ...
    GET_OBJ_WEIGHT(temp) -= GET_OBJ_WEIGHT(obj);
    if (temp->carried_by) {
        if(IS_CARRYING_W(temp->carried_by) < 0) {
  /*          sprintf(buf, "WEIGHT: PC: %s OBJ: %s OBJFROM: %s", 
                    GET_NAME(temp->carried_by), obj->short_description,
                    temp->short_description);
            log(buf); */
            IS_CARRYING_W(temp->carried_by) = number(5, 15);
        }
        else {
...
void update_object(struct obj_data *obj, int use)
{
    ...
    if (!SCRIPT_CHECK(obj, OTRIG_TIMER) && (GET_OBJ_TIMER(obj) == 1))
	GET_OBJ_TIMER(obj) = -1;
...
void update_char_objects(struct char_data *ch)
{
    ...
    /* AFTERLIFE AFFECT CHECK */
    if (GET_ALIGNMENT(ch) <= 0 && IS_AFFECTED(ch, AFF_AFTERLIFE)) {
	send_to_char
	    ("Your alignment has become such that your gods have taken "
	     "away your afterlife.\r\n", ch);
	REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_AFTERLIFE);
    }

    /* ENLIGHTENMENT AFFECT CHECK */
    if (GET_ALIGNMENT(ch) <= 0 && IS_AFFECTED(ch, AFF_ENLIGHT)) {
	send_to_char
	    ("Your alignment has become such that your gods have taken "
	     "away your enlightenment.\r\n", ch);
	REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_ENLIGHT);
    }
	...
	if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT)
		...
		if (i == 1) {
		    act("Your light begins to flicker and fade.", FALSE,
			ch, 0, 0, TO_CHAR);
		    act("$n's light begins to flicker and fade.", FALSE,
			ch, 0, 0, TO_ROOM);
		} else if (i == 0) {
		    act("Your light sputters out and dies.", FALSE, ch, 0,
			0, TO_CHAR);
		    act("$n's light sputters out and dies.", FALSE, ch, 0,
			0, TO_ROOM);
		    world[ch->in_room].light--;
		}
	    }
...
struct char_data *get_char_room_vis(struct char_data *ch, char *name)
{
    ...
    for (i = world[ch->in_room].people; i && j <= number;
	 i = i->next_in_room) if (isname(tmp, i->player.name))
	    if (CAN_SEE(ch, i) && !IS_AFFECTED(i, AFF_BURROW))
		if (++j == number)
		    return i;
...
struct obj_data *get_obj_vis(struct char_data *ch, char *name)
{
	...
	if (isname(tmp, i->name))
	    if (CAN_SEE_OBJ(ch, i)
		&& (!i->carried_by
		    || CAN_SEE(ch, i->carried_by))) if (++j == number)
		    return i;
...
struct char_data *get_player_idnum(int num)
{
    struct char_data *i;
    for (i = character_list; i; i = i->next)
	if (!IS_NPC(i) && (GET_IDNUM(i) == num))
	    return i;
    return NULL;
}
...
struct char_data *get_player(struct char_data *ch, char *name)
{
    ...
    for (i = character_list; i; i = i->next)
	if (!IS_NPC(i) && (GET_REAL_LEVEL(ch) >= GET_INVIS_LEV(i))
