...
/* increases the target's quest points */
ACMD(do_mqpadd)
{
    char_data *victim;
    char name[MAX_INPUT_LENGTH], amount[MAX_INPUT_LENGTH];
...
/*
    mob_log(ch, "WARNING: mqpadd command is depracated! Use: %actor.qpoints(amount-to-add)%");
*/  
    ...
    if (!*name || !*amount) {
        mob_log(ch, "mqpadd: too few arguments");
        return;  
    }

    if (*name == UID_CHAR) {
      if (!(victim = get_char(name))) {
        sprintf(buf, "mqpadd: victim (%s) does not exist",name);
        mob_log(ch, buf);
        return;
      }
    } else if (!(victim = get_char_vis(ch, name))) {
    ...
    GET_QPOINTS(ch) += qp_add;
/*
    if ((GET_QPOINTS(victim) += atoi(amount)) < 0) {
        mob_log(ch, "mqpadd subtracting more gold than character has");
        GET_QPOINTS(victim) = 0;  
    }
*/
}  
