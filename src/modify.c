...
ACMD(do_abilset)
{
   ...
   */
  if (!*name) {
    send_to_char("Syntax: abilset <name> '<ability>' <value>\r\n", ch);
    strcpy(help, "ability being one of the following:\n\r");
    for (i = 0; *abil_list[i] != '\n'; i++) {
      if (*abil_list[i] == '!')
	continue;
      sprintf(help + strlen(help), "%18s", abil_list[i]);
      if (i % 4 == 3) {
	strcat(help, "\r\n");
  ...
  if (!*argument) {
    ...
    send_to_char("Ability must be enclosed in: ''\n\r", ch);
    return;
  }
  /*
  ...
  help[qend - 1] = '\0';
  if ((abil = find_abil_num(help, abil_list)) <= 0) {
  ...
  sprintf(buf2, "%s changed %s's %s to %d.", GET_NAME(ch), GET_NAME(vict),
	  abil_list[abil], value);
  mudlog(buf2, BRF, -1, TRUE);

  set_ability(vict, subcmd, abil, value);
