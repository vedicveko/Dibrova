...
//    GET_OBJ_PLAYER(obj) = object.player;
...
int Obj_to_store_from(struct obj_data * obj, FILE * fl, int locate)
{
  ...
  if(!obj || obj == NULL) {
    log("No obj in Obj_to_store_from()");
    return 0;
  } 
  
// Cross your fingers and dot your eyes.
...
//  object.player = GET_OBJ_PLAYER(obj);
  for(taeller = 0; taeller < AF_ARRAY_MAX; taeller++)
...
void Crash_listrent(struct char_data * ch, char *name)
{
  ...
  if (!get_filename(name, fname, CRASH_FILE))
    return;
  if (!(fl = fopen(fname, "rb"))) {
    sprintf(buf5, "%s has no rent file.\r\n", name);
...
void auto_equip(struct char_data *ch, struct obj_data *obj, int locate)
{
  ...
  if (locate > 0) { /* was worn */
    ...
    case WEAR_THUMB_R:
    case WEAR_THUMB_L:
      if (!CAN_WEAR(obj,ITEM_WEAR_THUMB))
	locate = 0;
       break;
    case WEAR_EYES:
    ...
    case WEAR_FLOAT_1:
    case WEAR_FLOAT_2:
      if (!CAN_WEAR(obj, ITEM_WEAR_FLOAT_1))
        locate = 0;
      break;
    case WEAR_ANKLE:
...
void Crash_rentsave(struct char_data * ch, int cost)
{
  ...
  if (IS_AFFECTED(ch, AFF_DEATHCALL))
    REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_DEATHCALL);

  if (IS_AFFECTED(ch, AFF_UNDER_ATTACK))
    REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_UNDER_ATTACK);
		         ...
		         int display, int factor)
{
  ...
  if (display) {
    ...
    act(buf, FALSE, receptionist, 0, ch, TO_VICT);
    if (totalcost > GET_GOLD(ch) && GET_LEVEL(ch) <= 150) {
		         ...
		         int cmd, char *arg, int mode)
{
  ...
  /* The price depents on the place where you stay. */
  ...
  if (CMD_IS("rent")) {
    ...
    if (PLR_FLAGGED(ch, PLR_SELLER)) {
      send_to_char("You cannot rent until your auction is over.\r\n", ch);
      return TRUE;
    }
    if (mode == RENT_FACTOR)
    ...
    act(buf, FALSE, recep, 0, ch, TO_VICT);
    if (cost > GET_GOLD(ch) && GET_LEVEL(ch) <= 150) {
      act("$n tells you, '...which I see you can't afford.'",
	  FALSE, recep, 0, ch, TO_VICT);
      return TRUE;
    }
    ...
    sprintf(buf2, "Saving to(rent): %d\r\n", save_room);
    send_to_char(buf2, ch);
    extract_char(ch);

    save_char(ch, save_room);
    sprintf(tmpstr, "/cG%s exits the realm./c0\r\n", GET_NAME(ch));
//    write_event(tmpstr);
    if (GET_LEVEL(ch) < LVL_IMMORT) 
      for (d = descriptor_list ; d ; d = d->next)
        if (d->character && !PRF_FLAGGED(d->character, PRF_NOINFO) &&
            ch != d->character)
          send_to_char(tmpstr, d->character);
  } else { /* Offer */
    sprintf(buf, "$N explains \"I charge a fixed rental rate of %d coins "
                 "per day.\"", cost); 
    act(buf, FALSE, ch, 0, recep, TO_CHAR);
    act("$N explains rental terms to $n.", FALSE, ch, 0, recep, TO_ROOM);
    if (cost && (mode == RENT_FACTOR))
      Crash_rent_deadline(ch, recep, cost);
  }
  return TRUE;
