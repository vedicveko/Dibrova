...
/* local globals */
int port; /* moved from main so it can be used in SHOW STAT */
int most_on;
...
void newbie_point_update(void);	/* In limits.c */
...
void proc_color(char *inbuf, int color_lvl); /* color.c */
...
*/
  ...
  if (!(pulse % (88 * PASSES_PER_SEC)))
    //broadcast_tips();

//  if (!(pulse % (120 * PASSES_PER_SEC)))
//    align_good(); 

//  if (!(pulse % (120 * PASSES_PER_SEC)))
//    align_evil(); 

  if (!(pulse % (5 * 60 * PASSES_PER_SEC))) {
    save_clans(); 
    record_usage();
  }


  if (!(pulse % (SECS_PER_MUD_HOUR/2))){
    beware_lightning();
  }
  ...
  if (!(pulse % (21 * PASSES_PER_SEC))) {
    titan_absorbtion();

  }
  if (!(pulse % (45 * PASSES_PER_SEC))) {
    vamp_sun_dam();
    sector_affects();
  }
  ...
  if (d->showstr_count) {
    ...
    *prompt = '\0';
    ...
    if ((PRF_FLAGGED(d->character, PRF_DISPMANA) &&
        (CAN_CAST(d->character) || CAN_PRAY(d->character))) ||
         GET_CLASS(d->character) >= 9)
      sprintf(prompt, "%s%dM ", prompt, GET_MANA(d->character));

    if ((PRF_FLAGGED(d->character, PRF_DISPVIM) &&
         GET_CLASS(d->character)==CLASS_DRUID) || GET_CLASS(d->character)>=9)
      sprintf(prompt, "%s%dI ", prompt, GET_VIM(d->character));

    if ((PRF_FLAGGED(d->character, PRF_DISPQI) &&
        GET_CLASS(d->character)==CLASS_MONK) ||
        GET_CLASS(d->character) >= 9)
      sprintf(prompt, "%s%dQ ", prompt, GET_QI(d->character));
    ...
    if (PRF_FLAGGED(d->character, PRF_DISPXTOL))
      if (GET_LEVEL(d->character)<LVL_IMMORT-1)
        sprintf(prompt, "%s[%d to level] ", prompt,
                level_exp(GET_CLASS(d->character),
                GET_LEVEL(d->character) + 1) - GET_EXP(d->character));

    if (PRF_FLAGGED(d->character, PRF_DISPPCT))
      sprintf(prompt, "%s[%s %d%%] ", prompt, GET_NAME(d->character), GET_HIT(d->character)*100/GET_MAX_HIT(d->character));

    if (PRF_FLAGGED(d->character, PRF_DISPTANK) && d->character->master &&
        IS_AFFECTED(d->character, AFF_GROUP))
      sprintf(prompt, "%s[%s %d%%] ", prompt, GET_NAME(d->character->master),
              GET_HIT(d->character->master)*100/
              GET_MAX_HIT(d->character->master));

    if (PRF_FLAGGED(d->character, PRF_DISPMOBP) && FIGHTING(d->character) &&
        GET_MAX_HIT(FIGHTING(d->character)))
      sprintf(prompt, "%s[%s %d%%] ", prompt,
              GET_NAME(FIGHTING(d->character)),
              (GET_HIT(FIGHTING(d->character)) * 100)/
              GET_MAX_HIT(FIGHTING(d->character)));

    if (strlen(prompt)>30 && PRF_FLAGGED(d->character, PRF_DISPSHORT))
      strcat(prompt, "\r\n> ");
    else
      strcat(prompt, "> ");
...
//  extern char *GREETINGS;
  ...
  if (sockets_connected >= max_players) {
    write_to_descriptor(desc, "Sorry, Dibrova is full right now... please try again later!\r\n");
    CLOSE_SOCKET(desc);
    return 0;
  }
  ...
  if (isbanned(newd->host) == BAN_ALL) {
    ...
    mudlog(buf2, CMP, LVL_ADMIN, TRUE);
    free(newd);
    return 0;
  }
#if 0
  ...
  mudlog(buf2, CMP, LVL_ADMIN, FALSE);
#endif
...
int process_output(struct descriptor_data *t)
{
  ...
  /* Storm -- this is an attempt to make sure we don't crash through strlen
   * in proc_color...*/
...
//      quest_purge(d->character->carrying);
    ...
    mudlog("Losing descriptor without char.", CMP, LVL_IMPL+1, TRUE);
...
void align_good()
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (IS_AFFECTED(d->character, AFF_ALIGN_GOOD) &&
        GET_LEVEL(d->character) < LVL_IMMORT) {
    GET_ALIGNMENT(d->character) += number(50, 135);
    send_to_char("You feel yourself changing in temperament slightly.\r\n",
                  d->character);
    }
  }
} 
...
void align_evil()
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (IS_AFFECTED(d->character, AFF_ALIGN_EVIL) &&
        GET_LEVEL(d->character) < LVL_IMMORT) {
    GET_ALIGNMENT(d->character) -= number(50, 135);
    send_to_char("You feel yourself changing in temperament slightly.\r\n",
                  d->character);
...
void broadcast_tips() {
  ...
  /* Grab a random tip... */
  sprintf(tip, "/cMTIP -> %s/c0", get_random_tip());

  /* Send it to everyone who wants it. */
  for (d=descriptor_list; d; d=d->next) {
    if (STATE(d)==CON_PLAYING
           && PRF_FLAGGED(d->character, PRF_SHOWTIPS)
           && GET_POS(d->character)!=POS_FIGHTING)
             send_to_char(tip, d->character);
  }
}
...
void send_to_arena(char *messg) {
  ...
  if (messg) {
    sprintf(armsg, "/cRARENA:: %s/c0", messg);
    for (d = descriptor_list; d; d = d->next)
      if (!d->connected && d->descriptor && 
          PRF_FLAGGED(d->character, PRF_ARENA))
        SEND_TO_Q(armsg, d);
  }
}

/* Storm --7/30/98 -- send to all players of a certain level or higher. */
void send_to_level(char *messg, int level) {
  struct descriptor_data *i;

  if (messg)
    for (i = descriptor_list; i; i= i->next)
      if (!i->connected && GET_LEVEL(i->character) >= level)
        SEND_TO_Q(messg, i);
}
...
void send_to_pkillers(char *messg) {
  struct descriptor_data *i;
 
  if (messg)
    for (i = descriptor_list; i; i= i->next)
      if (!i->connected && PLR_FLAGGED(i->character, PLR_PKILL_OK))
        SEND_TO_Q(messg, i);
}
...
#endif
void old_act(char *str, int hide_invisible, struct char_data *ch,
...
!ROOM_FLAGGED(i->character->in_room, ROOM_SOUNDPROOF)) {
  ...
  else if (obj && obj->in_room != NOWHERE && obj->in_room < top_of_world) {
    to = world[obj->in_room].people;
  } else {
      if(ch)
        sprintf(buf, "SYSERR: no valid target to act()!: %s", GET_NAME(ch)); 
      else if(obj)
        sprintf(buf, "SYSERR: no valid target to act()!: obj");
      else
        sprintf(buf, "SYSERR: no valid target to act()!: %d", type);
...
void new_perform_act(const char *orig, struct char_data *ch, struct obj_data *obj,
	 ...
	 struct obj_data *obj, const void *vict_obj, int type)
{
  ...
  if (type == TO_CHAR) {
    if (ch && SENDOK(ch))
      new_perform_act(str, ch, obj, vict_obj, ch);
    return;
  }

  if (type == TO_VICT) {
    if ((to = (const struct char_data *) vict_obj) != NULL && SENDOK(to))
      new_perform_act(str, ch, obj, vict_obj, to);
    return;
  }
  if (type == TO_GMOTE)
  {
	  for (i = descriptor_list; i; i = i->next)
	  {
		  ...
		  {
			  send_to_char(CCYEL(i->character, C_NRM), i->character);
			  new_perform_act(str, ch, obj, vict_obj, i->character);
			  send_to_char(CCNRM(i->character, C_NRM), i->character);
		  }
	  }
	  return;
