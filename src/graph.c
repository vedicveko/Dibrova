...
void hunt_victim(struct char_data * ch)
{
  ...
  if(GET_CLASS(ch) == CLASS_NPC_MAGE) {
  if (ch->in_room != HUNTING(ch)->in_room) {
    cast_spell(ch, HUNTING(ch), NULL, SPELL_PORTAL, ABT_SPELL, FALSE);
    command_interpreter(ch, "jump portal");
    if (ch->in_room == HUNTING(ch)->in_room) {
       do_say(ch, "Who wants some!?", 0, 0);
...
void move_hunters() {
  ...
  for (mob = character_list; mob; mob = mob->next)
    if (IS_NPC(mob) && HUNTING(mob) && !FIGHTING(mob) && !number(0,2) &&
        (GET_LEVEL(HUNTING(mob))<LVL_IMMORT ||
         !PRF_FLAGGED(HUNTING(mob), PRF_NOHASSLE)))
      hunt_victim(mob);
